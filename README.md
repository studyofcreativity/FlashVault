...
Tú entras en **Panel privado**, inicias sesión y primero eliges uno de estos dos tipos:

- **Juego Flash de un solo archivo**: un único `.swf`.
- **Juego Flash con múltiples recursos**: un `.zip` que contiene el `.swf` y sus imágenes, sonidos, XML, carpetas y demás recursos. FlashVault conserva la estructura de carpetas y detecta el SWF principal para abrirlo con Ruffle.

En ambos casos también publicas nombre, descripción y portada. Para los paquetes, ejecuta `supabase-migration-multi-resource.sql` si ya tenías instalada una versión anterior de FlashVault.
